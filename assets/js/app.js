...
$(function () {
  ...
  $("#add_pro_cart_btn").on("click", function (e) {
    $(".cart_error").addClass("hide");
    $.post(
      BASE_URL + "order/addProCart",
      $("#add_pro_cart").serialize(),
      function (data) {
  ...
  $("#order_cart_div").on("click", ".del_cart_pro", function (e) {
    ...
    if (confirm("Are you sure?")) {
      $.post(
        BASE_URL + "order/delPro",
        { raw_id: _row_id, combine_id: _free_id, pid: _pid },
        function (data) {
  ...
  $("#log_print").on("click", function () {
    ...
    if ($.trim(_start) == "" || $.trim(_end) == "") {
      ...
      var win = window.open(
        BASE_URL + "logs/report/" + _location + "/" + _start + "/" + _end,
        "_blank"
      );
      if (win) {
  ...
  $("#patient_refferral_name").autocomplete({
    ...
    source: function (request, response) {
      ...
      $.getJSON(
        BASE_URL + "patient/getActivePatients",
        request,
